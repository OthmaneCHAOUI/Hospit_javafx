...
public class TestMedicalDAO {
    public static  List<TestMedical> getTests(String cniePatient, int idDoctor) {
        List<TestMedical> tests = new ArrayList<>();
        String sql = "SELECT id,type_test,resultat,date_test,type_doctor,statut FROM Test_Medical WHERE cnie_patient = ? AND id_doctor = ?";
